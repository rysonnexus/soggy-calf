        ...
        {digits.map((d, i) => {
          ...
          return (
            <button
              key={i}
              type="button"
              onClick={() => setPin((p) => (p.length < PIN_LENGTH ? p + d : p))}
  ...
  return (
    <div className="card w-full max-w-sm sm:max-w-md">
      <div className="text-center mb-6">
        <FontAwesomeIcon
          icon={faKey}
          className="text-tavern-amber text-3xl mb-3"
        />
        <h2 className="font-display text-lg sm:text-xl font-bold text-tavern-gold">
          Change Your PIN
        </h2>
        <p className="text-tavern-muted text-xs sm:text-sm mt-1">{subtitle}</p>
      </div>
      ...
      {showCancel && (
        <button
          type="button"
          onClick={onCancel}
          className="text-tavern-muted text-sm underline w-full text-center mt-4"
        >
          Cancel
        </button>
      )}
