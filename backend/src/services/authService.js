...
async function rotateRefreshToken(oldRawToken) {
  const stored = await prisma.refreshToken.findUnique({ where: { token: oldRawToken } })
  ...
  return { ...tokens, user: { id: user.id, username: user.username, role: user.role, mustChangePIN: user.mustChangePIN } }
}
...
async function loginWithPIN(username, pin) {
  ...
  if (!user) {
    ...
    await bcrypt.compare('0000', '$2b$12$invalidhashinvalid.invalidinvalidinval')
...
module.exports = { hashPIN, loginWithPIN, issueTokens, rotateRefreshToken, revokeRefreshToken, changePIN, prisma }
